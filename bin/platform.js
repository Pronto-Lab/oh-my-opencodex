 ...
 * @returns {string} Package name like "oh-my-opencodex-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `oh-my-opencodex-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "oh-my-opencodex-darwin-arm64/bin/oh-my-opencodex"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/oh-my-opencodex${ext}`;
}
