...
export async function runCliInstaller(args: InstallArgs, version: string): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo(
      "Usage: bunx oh-my-opencodex install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    )
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Adding oh-my-opencodex plugin...")
  ...
  printStep(step++, totalSteps, "Writing oh-my-opencodex configuration...")
  ...
  console.log(
    `  ${color.dim("gh api --silent --method PUT /user/starred/code-yeongyu/oh-my-opencodex >/dev/null 2>&1 || true")}`,
  )
  console.log()
