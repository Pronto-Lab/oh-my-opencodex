...
/**
 * Get the cache directory for oh-my-opencodex binaries.
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[oh-my-opencodex] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[oh-my-opencodex] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[oh-my-opencodex] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[oh-my-opencodex] Comment checking disabled.`)
    return null
  }
}
