# Contributing to Oh My OpenCodex

First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to oh-my-opencodex.
...
git clone https://github.com/code-yeongyu/oh-my-opencodex.git
cd oh-my-opencodex
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   {
     "plugin": [
       "file:///absolute/path/to/oh-my-opencodex/dist/index.js"
     ]
   }
   ```
   
   For example, if your project is at `/Users/yourname/projects/oh-my-opencodex`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/oh-my-opencodex/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"oh-my-opencodex"` from the plugin array if it exists, to avoid conflicts with the npm version.
...
│   └── index.ts       # Main plugin entry (OhMyOpenCodexPlugin)
...
Thank you for contributing to Oh My OpenCodex! Your efforts help make AI-assisted coding better for everyone.
